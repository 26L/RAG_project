...
> 날짜별 진행 기록. 결론은 [RESULT.md](RESULT.md), 다음 할 일은 문서 하단 TODO.
...
**GraphRAG + 로컬 임베딩 + 진단**
- GraphRAG 논문 정독·번역([GUIDE.md](GUIDE.md)), 구현 `methods/graphrag.py` *(12:21)* — MAX_TOKENS·nested async·임베딩 quota 해결.
...
- 그래프 품질 개선 설계안 [DESIGN.md](DESIGN.md) 작성.
...
- **용도별 2 프로필**(§11): 범용(hybrid) / 커뮤니티(graph+vector). config·docs/RESULT.md.
...
### 2026-07-21~22 (재설계 Plan A 실행 — 짧은답 프롬프트 + n=400)
**시도·완료한 것**
- **짧은답 프롬프트**(§10.12) — `answer_style: short` 스위치 하나로 전 기법 적용
  ([_common.py] `apply_answer_style` / [hipporag.py] `_ANSWER_PROMPT_SHORT`). EM/F1 을 하니스에 정식 배선.
  - **원칙 교정(중요)**: 초안 *"근거 없어도 최선 추정을 내라"* 는 **환각 지시**라 폐기.
    최종안 = 사전지식 금지 + 컨텍스트 근거만 + 근거 없으면 `NO_ANSWER` **기권**(오답과 분리 집계).
    검색 실패를 모델 암기력으로 덮으면 **검색 성능이 아니라 암기력**을 재게 된다.
  - 결과(n=100): F1 0.19~0.21 → **0.46~0.55**, EM 0.03~0.12 → **0.37~0.45**, 단어수 20.4 → 1.7.
    standard 0.552 ≈ VLDB VanillaRAG 50.8, HippoRAG2 0.521 ≈ 논문 50.3 — **순서 관계까지 재현**.
  - **예상 밖 부수효과**: 같은 n=100 에서 McNemar 유의쌍 **0/10 → 3/10**. 문장형이 judge 를
    관대하게 만들어 기법 차이를 뭉개고 있었다 → 판별력 개선의 실체는 **잡음 감소**.
- **n=400 확대** — [scripts/build_hotpot_bench.py](../scripts/build_hotpot_bench.py) 신설(`--n`).
  기존 n=100 을 **코퍼스 바이트 일치(diff 0)** 로 재현 검증 후 확대(앞 100문항은 동일 prefix).
  문항 400 · 고유문서 3,957 · 578K 토큰. 경로 분리로 n=100 자산 보존.
- **★ 평면 3종 n=400 — 프로젝트 최초의 통계적으로 유의한 우열**
  hybrid **0.623** [.575,.670] / standard **0.613** [.565,.657] / bm25 0.532 [.485,.583].
  McNemar: hybrid>bm25 **p=.00001**, standard>bm25 **p=.001**, hybrid vs standard **p=.63(차이없음)**.
  → **§10.7 교정**: "세 방법 전부 동률"은 결론이 아니라 **검정력 부족**이었다(취소선으로 보존).
  → **함의**: 하이브리드 이득은 BM25 를 더해서가 아니라 **벡터검색 자체**에서 온다. 회사 코퍼스의
  "하이브리드 > 단일"은 여기서 재현 안 됨 → §10.7 "도메인 특이적" 판단 **강화**.
- **도구 신설**: [scripts/stats_report.py](../scripts/stats_report.py)(부트스트랩 CI + McNemar — 애드혹이던 통계를 스크립트화),
  [scripts/judge_validation.py](../scripts/judge_validation.py)(judge 사람 라벨 검증 — 앵커링 방지 위해 시트에 judge 판정 미노출, 과대/과소 방향 분리 집계).
- **비용 산출(실측 토큰)**: n=400 전체 ≈ **$9**(평면 $0.51 + graphrag_e2b $2.83 + hipporag $5.40).
  HippoRAG 가 60% — 문단당 LLM 2회(NER 217tok + 트리플 505tok) + 질의당 필터 2,253tok.
  **NIM 무료 혼합안은 검토 후 기각** — 추출기 교체는 §10.5 전례상 그래프를 바꿔 n=100 과 비교 불가.
  **전 구간 Gemini 단일**로 confound 제거(승인 예산 ₩20,000).
- **Neo4j 확인**: `src/ragbench/` 내 참조 **0건** — 시각화 전용이며 파이프라인에 무관.
  지금까지 결과 중 neo4j 때문에 잘못된 것 없음. HotpotQA n=100 그래프 적재(엔티티 8,767·청크 992·
  관계 10,658; `MENTIONS` 8,767 제외 시 실질 의미관계 1,891 — `located in` 367 등).
- 아티팩트: **평가표**(CI 막대·McNemar·교정기록·한계 명시).

**★ 실패 기록 — `graphrag_e2b` n=400 인덱싱 행(hang), 2h42m 손실**
- **증상**: 프로세스는 살아 있고 소켓도 ESTAB 인데 **진행이 0**.
  실측 — `rchar` 가 68분간 **+224바이트**(125,871,903 → 125,872,127), `syscr` 45초간 +2,
  CPU 45초간 +0.07초, wchan `ep_poll`. 즉 **응답 없는 연결을 타임아웃 없이 붙들고 무한 대기.**
- **원인**: 프로세스를 죽이자 버퍼가 flush 되며 드러남 — Gemini 가 **502 Bad Gateway / 503
  UNAVAILABLE**(구글 서버측 일시 장애)을 반환, LlamaIndex 가 재시도하다 연결이 물림.
  진행바는 `Applying transformations: 0%|  | 0/1` 에서 정지. **우리 코드 버그가 아니라
  공급자 장애 + 타임아웃 부재의 조합.**
- **★ 진단 실수(반성)**: 00:27·02:00 보고에서 "정상 가동 중"이라 했는데 근거가 **ESTAB 소켓뿐**이었다.
  연결이 살아있는 것과 데이터가 흐르는 것은 다르다. **`rchar`/`syscr` 추이를 비교했으면 1시간 일찍
  잡았다.** → 앞으로 장기 작업의 생존 판정은 **누적 I/O 증가분**으로 한다(소켓 상태 금지).
- **손실**: 실진행 ~1.5h + 대기 1.2h, 중간 결과 **0**(체크포인트 없음), API 비용 ~$1~2 회수 불가.
- 로그 보존: `scratchpad/graph400_hang.log`.
- 정리 상태: 프로세스 전부 종료, 미완 `storage_hotpot400/hipporag`(24K) 삭제,
  `storage_hotpot400/` 에는 평면 3종만 남김. 작업트리 깨끗.

**미완 (다음 세션 이어받을 것)**
1. **[선행·필수] 인덱싱 견고화** — 이걸 먼저 넣지 않으면 3~4시간을 또 통째로 날린다.
   - **LLM 요청 타임아웃** 설정(현재 무한 대기 → 한 번 물리면 전체 정지)
   - **배치 체크포인트** — N건마다 중간 저장, 죽어도 재개
   - **진행 로그를 라인버퍼로** — `| tail -3` 탓에 완료 전까지 진행 상황이 안 보였다(행 감지 지연의 직접 원인)
2. `graphrag_e2b` n=400 인덱싱 재실행 → 평가 → 평가표 갱신
3. `hipporag` n=400. **착수 전 실제 출력토큰으로 비용 재산출**(₩20,000 초과 전망이면 중단·보고)
4. judge 사람 라벨 검증 50문항 — 도구([scripts/judge_validation.py](../scripts/judge_validation.py)) 준비 완료, 비용 0
