...
**미완 (다음 세션 이어받을 것)**
- `graphrag_e2b` n=400 인덱싱 **2시간 23분째 진행 중**(추출 3,957건). 완료 시 평가 → 결과표 갱신.
- 그 뒤 `hipporag` n=400. **착수 전 실제 출력토큰으로 비용 재산출**(₩20,000 초과 전망이면 중단·보고).
- ⚠️ **체크포인트 없음** — 인덱싱이 중간에 죽으면 전량 재실행. 다음 구현 시 배치 저장 추가할 것.
